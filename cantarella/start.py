...
from config import API_ID, API_HASH, ERROR_MESSAGE, OWNER_ID
...
async def show_thumb_menu(client: Client, callback_query: CallbackQuery):
    ...
    if thumb:
        ...
        preview_btn = [InlineKeyboardButton(small_caps("👁️ View"), callback_data="view_thumb")]
        delete_btn = [InlineKeyboardButton(small_caps("🗑️ Delete"), callback_data="remove_thumb")]
    else:
...
┃ <b>📅 {small_caps('Joined')}:</b> {joined.strftime('%Y-%m-%d')}        ┃
...
async def receive_text_input(client: Client, message: Message):
    ...
    if user_id in UserState.waiting_for_repl_word_target:
        UserState.waiting_for_repl_word_target[user_id] = False
    ...
    # If not in any state, check for Telegram link (original save functionality)
    if "https://t.me/" in message.text:
        # Original save logic here (keep your existing save function code)
        # I'm keeping it as is since it's long
        pass